...
export default function AdminBell(){
  ...
  useEffect(()=>{
    ...
    const load = async()=>{
      try{
        const { data } = await supabase.rpc('get_withdrawal_counts')
        if(alive) setCount(data?.pending||0)
    ...
    return ()=>{ alive=false; try{ if(ch) (supabase as any).removeChannel(ch) }catch{} }
  },[isAdmin])
