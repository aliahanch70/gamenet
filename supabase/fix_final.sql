-- FIX FINAL — قطع نشت پروفایل + هماهنگ‌سازی پارامتر برداشت
-- اجرا: Supabase Dashboard → SQL Editor → Paste کل فایل → Run
-- اگر خطا دیدی، هر بخش را جداگانه Run کن (Supabase گاهی کل تراکنش را rollback می‌کند)

-- 1) profiles: هر کاربر فقط خودش را ببیند (نشتی که تست نشان داد)
alter table public.profiles enable row level security;

-- حذف همه پالیسی‌های شل قبلی — با هر نامی که باشند
...
-- تریگر لایه دوم (مقاوم حتی اگر RLS دور زده شود)
...
-- 2) withdrawals: اسم پارامتر باید p_withdrawal_id باشد (همین را فرانت می‌فرستد)
-- live الان p_id دارد → p_wd_id → 404. هر امضای موجود را پاک و درست را بساز.
...
-- رفرش کش PostgREST (Supabase معمولاً خودکار می‌کند، اما برای اطمینان)
